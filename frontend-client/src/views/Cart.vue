<template>
	<div class="cart-box">
		<s-header :name="'Shopping cart'" :noback="true"></s-header>
		<div class="cart-body">
...
export default {
	...
	setup() {
		...
		const onChange = async (value, detail) => {
			if (value > 5) {
				Toast.fail('The maximum purchase quantity exceeds a single product')
				return
			}
			if (value < 1) {
				Toast.fail('Products must not be less than 0')
				return
			}
			if (state.list.filter((item) => item.cartItemId == detail.name)[0].goodsCount == value)
				return
			Toast.loading({ message: 'Editing...', forbidClick: true })
			const params = {
		...
		const onSubmit = async () => {
			if (state.result.length == 0) {
				Toast.fail('Please select the product to settle')
				return
			}
