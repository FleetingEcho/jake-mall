	...
	<div class="add">
		<el-card class="add-container">
			<el-form
				...
				class="goodForm"
			>
				<el-form-item required label="Product Category ">
					<el-cascader
				...
				<el-form-item label="Product Introduction " prop="goodsIntro">
					<el-input
						style="width: 300px"
						type="textarea"
						v-model="goodForm.goodsIntro"
						placeholder="Please fill in  product introduction (max to 100 chars)"
					></el-input>
				</el-form-item>
				<el-form-item label="Product Price" prop="originalPrice">
					<el-input
						type="number"
						min="0"
						style="width: 300px"
						v-model="goodForm.originalPrice"
						placeholder="Please fill in  product price"
					></el-input>
				</el-form-item>
				<el-form-item label="Product Selling Price" prop="sellingPrice">
					<el-input
						type="number"
						min="0"
						style="width: 300px"
				...
				<el-form-item label="Product Inventory" prop="stockNum">
					<el-input
						type="number"
						min="0"
						style="width: 300px"
						v-model="goodForm.stockNum"
						placeholder="Please fill in product inventory"
					></el-input>
				</el-form-item>
				<el-form-item label="Product Tag" prop="tag">
					<el-input
						style="width: 300px"
						v-model="goodForm.tag"
						placeholder="Please fill in product tag"
					></el-input>
				</el-form-item>
				<el-form-item label="Shelf Status" prop="goodsSellStatus">
					<el-radio-group v-model="goodForm.goodsSellStatus">
						<el-radio label="0">On shelf</el-radio>
						<el-radio label="1">Off shelf</el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item required label="Product Cover Image" prop="goodsCoverImg">
					<el-upload
...
export default {
	...
	setup() {
		...
		const state = reactive({
			...
			rules: {
				goodsCoverImg: [
					{ required: 'true', message: 'Please upload main image', trigger: ['change'] },
				],
				goodsName: [
					{
						required: 'true',
				...
				stockNum: [
					{
						required: 'true',
						message: 'Please fill in Product Inventory',
						trigger: ['change'],
					},
				],
			},
			...
			goodRef.value.validate((vaild) => {
				if (vaild) {
					...
					if (params.goodsName.length > 128) {
						ElMessage.error('Product name no more than 128 chars')
						return
					}
					if (params.goodsIntro.length > 200) {
						ElMessage.error('Product introduction no more than 200 chars')
						return
					}
					if (params.tag.length > 16) {
						ElMessage.error('Product tags no more than 16 chars')
						return
					}
					...
					httpOption('/goods', params).then(() => {
						ElMessage.success(id ? 'Modified successfully' : 'Added successfully')
		...
		const handleBeforeUpload = (file) => {
			...
			if (!['jpg', 'jpeg', 'png'].includes(sufix)) {
				ElMessage.error('Please upload jpg、jpeg、png format image')
				return false
			}
		}
